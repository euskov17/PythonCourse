...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    return {page["UserID"] for page in records if page["EventTime"] > ts}
...
def get_events_by_device_type(
    ...
    """
    return [page for page in records if page["DeviceType"] is device_type]
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    return [page["RegionID"] if page["RegionID"] is not None else DEFAULT_REGION_ID for page in records]
...
def get_keys_where_value_is_not_none(r: tp.Mapping[str, tp.Any]) -> list[str]:
    ...
    """
    return [field for field in r if r[field] is not None]
